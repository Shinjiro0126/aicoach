...
/**
 * 飛び石の道のり(デザイン01/04)。
 * 週アラインの14日(上段=先週、下段=今週)をS字2段の飛び石で描画する。
 * 歩いた日=濃い石、報告した日=輪郭の薄い石、おやすみ救済=葉の石、
 * これから=点線、目標開始前=石なし(水面のまま)。
 * 下段は左岸の週の旗へ向かって渡っていき、今日の石にはホトリ(bust)が立つ。
...
// ---- 通常レイアウト(週アライン14日・S字2段) ----
// 上段: days[0..6](先週・左→右)/ 下段: days[7..13](今週・右→左で左岸の週の旗へ向かう)。
// 上段末尾はやや上げて、下段右端(週初日)に立つホトリと上段の石が重ならないようにする
const TOP_POSITIONS = [
  { x: 26, y: 44 },
  { x: 67, y: 38 },
  { x: 108, y: 44 },
...
const FULL_POSITIONS = [...TOP_POSITIONS, ...BOTTOM_POSITIONS];
...
function FullJourneySvg({ days, colors }: { days: JourneyDay[]; colors: StoneColors }) {
  return (
    ...
    <Svg viewBox="0 0 316 132" width="100%" height="100%">
      ...
      />
      {days.slice(0, FULL_POSITIONS.length).map((day, i) => (
        <StoneShape key={day.dateKey} day={day} cx={FULL_POSITIONS[i].x} cy={FULL_POSITIONS[i].y} colors={colors} />
      ))}
      {/* 週の旗(左岸)= 下段(今週)の到達点 */}
...
export function JourneyStones({ days, weekNo, daysToFlag, reached, coldStart, caption }: JourneyStonesProps) {
  ...
  // 今日の石の位置は todayIndex から算出(通常レイアウトでは今日は下段=今週のどこかを移動していく)
  const layoutPositions = coldStart ? COLD_POSITIONS : FULL_POSITIONS;
  const todayPos = layoutPositions[Math.max(0, Math.min(todayIndex, layoutPositions.length - 1))];
